...
const Navbar = () => {
  ...
  // Helper to handle hash links if on home page, or redirect to home with hash
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'glass-nav h-16' : 'bg-transparent h-20'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
        <div className="flex items-center justify-between h-full">
          <Link to="/" className={`flex items-center gap-2 group cursor-pointer transition-opacity duration-300 ${mobileMenuOpen ? 'md:opacity-100 opacity-0 pointer-events-none md:pointer-events-auto' : 'opacity-100'}`}>
          ...
          <div className="hidden md:flex items-center gap-6">
            {['Process', 'Solutions'].map((item) => {
                ...
                // If it's a hash link on the same page, we use a standard anchor for scrolling
                if (location.pathname === '/' && href.startsWith('#')) {
                    return (
                        <a key={item} href={href} className="text-sm font-medium text-slate-300 hover:text-white transition-colors">
                            {item}
                        </a>
                    );
                }
                // Otherwise navigate to home with hash (Router handles this if setup correctly, or we can use Link)
                return (
                    <Link key={item} to={`/${href.startsWith('/') ? href.substring(1) : href}`} className="text-sm font-medium text-slate-300 hover:text-white transition-colors">
                        {item}
                    </Link>
                );
            })}
            ...
            <a href={location.pathname === '/' ? "#contact" : "/#contact"} className="relative px-6 py-2.5 bg-brand-600 text-white rounded-full text-sm font-semibold hover:bg-brand-500 transition-all duration-300 hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(6,182,212,0.5)] animate-pulse">
              Contact Us
            </a>
          </div>

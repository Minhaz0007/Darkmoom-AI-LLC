...
const FeaturedCaseStudy = () => (
    <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto reveal-on-scroll">
        <div className="glass-card p-8 md:p-12 rounded-3xl border border-brand-500/20 relative overflow-hidden group hover:border-brand-500/40 transition-all duration-500">
             {/* Background glow */}
            <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-brand-400/10 rounded-full blur-3xl pointer-events-none group-hover:bg-brand-400/20 transition-all duration-500"></div>

            <div className="relative z-10 flex flex-col lg:flex-row items-center gap-10">
                <div className="flex-1 text-center lg:text-left">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-500/10 rounded-full mb-6 border border-brand-500/20">
                        <span className="w-2 h-2 rounded-full bg-brand-400 animate-pulse"></span>
                        <span className="text-xs font-bold uppercase tracking-wider text-brand-400">Featured Case Study</span>
                    </div>
                    <h3 className="font-display text-3xl md:text-4xl font-bold text-white mb-4">Multi-Refinery Production Intelligence</h3>
                    <p className="text-slate-400 text-lg mb-8 leading-relaxed">
                        See how we unified data from 3 refineries into a single real-time dashboard, reducing reporting time by 90% and uncovering hidden energy costs.
                    </p>
                    <Link to="/case-study" className="inline-flex items-center gap-2 px-6 py-3 bg-brand-600/20 text-brand-400 font-bold rounded-full hover:bg-brand-600 hover:text-white transition-all duration-300 group-hover:translate-x-1 border border-brand-500/20 hover:border-brand-500">
                        Read Case Study <ArrowRight className="h-5 w-5" />
                    </Link>
                </div>
                <div className="flex-1 w-full max-w-lg">
                     {/* Mini preview representation */}
                     <div className="aspect-video bg-slate-950/80 rounded-xl border border-slate-700 overflow-hidden relative group-hover:border-brand-500/30 transition-colors shadow-2xl">
                        {/* Mock Dashboard UI */}
                        ...
                        <div className="p-4 pt-12 grid grid-cols-2 gap-3 h-full">
                            <div className="bg-slate-800/50 rounded border border-slate-700/50"></div>
                            <div className="bg-slate-800/50 rounded border border-slate-700/50"></div>
                            <div className="col-span-2 bg-slate-800/50 rounded border border-slate-700/50 mt-auto h-2/3 flex items-end px-2 pb-2 gap-1">
                                 {[40, 70, 50, 90, 60, 80, 50].map((h, i) => (
                                     <div key={i} className="flex-1 bg-brand-500/40 rounded-t" style={{ height: `${h}%` }}></div>
                                 ))}
                            </div>
                        </div>
                        {/* Overlay */}
